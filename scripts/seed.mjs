...
function rawAsset(record) {
  if (record.rawPath == null) return null;
  if (typeof record.rawPath !== "string" || !record.rawPath.startsWith("data/raw/")) {
    throw new Error(`${record.id}: rawPath must stay under data/raw`);
  }
  const assetPath = path.resolve(root, record.rawPath);
  if (!assetPath.startsWith(path.join(root, "data/raw") + path.sep) || !fs.existsSync(assetPath)) {
    throw new Error(`${record.id}: raw asset is missing`);
  }
  const contents = fs.readFileSync(assetPath);
  if (sha256(contents) !== record.contentHash) throw new Error(`${record.id}: raw asset SHA-256 does not match registry`);
  return contents;
}
