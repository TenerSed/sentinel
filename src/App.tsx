...
export default function App() {
  ...
  return <main className="recent-shell">
    <header className="recent-header"><p className="eyebrow">LAMPLIGHTER / OFFLINE DEMO</p><h1>{activeTab === "chat" ? "Ask about public records" : activeTab === "curated" ? "Curated government updates" : activeTab === "account" ? "Local account settings" : "Recent government updates"}</h1><p>{activeTab === "chat" ? "Answers stay inside the coverage and surfaced records you selected." : activeTab === "curated" ? "Offline, local personalization over source-grounded updates in the coverage you selected." : activeTab === "account" ? "Controls for this browser only. There is no sign-in, sync, or server profile." : "Source-grounded public records for the coverage you selected."}</p>
    ...
    {activeTab === "recent" ? <section className="evidence-grid" aria-label="Recent government updates"><section className="record-list" aria-labelledby="recent-heading"><div className="panel-heading"><p className="eyebrow">VERIFIED UPDATES</p><h2 id="recent-heading">Recent in {label}</h2><p>{records.length} {records.length === 1 ? "update" : "updates"}</p></div>
      ...
      {rankedRecords.length ? rankedRecords.map(({ record, reason, starter }) => <button type="button" key={record.id} className={`record-row ${record.id === selected?.id ? "selected" : ""}`} aria-pressed={record.id === selected?.id} onClick={() => { setSelectedId(record.id); setCopyError(false); setFutureMessage(""); }}><span className="record-meta">{record.locationLabel} · {sourceLabel[record.sourceKind]} · <time dateTime={record.publishedAt}>{dateText(record.publishedAt)}</time></span><span className="record-tags"><span>{updateLabel[record.updateType]}</span><span>{sourceLabel[record.sourceKind]}</span></span><strong>{record.title}</strong><span className="quote-preview">“{record.exactQuote}”</span><span className="curated-why"><strong>Why this</strong> {starter ? `Starter view · ${reason.recency}` : [reason.topic && `topic: ${reason.topic}`, reason.action, reason.recency].filter(Boolean).join(" · ")}</span><span className="record-locator">{locatorText(record.locator)}</span></button>) : <div className="empty-state"><h2>No verified updates for {label}</h2><p>Lamplighter will not broaden coverage or substitute other records. Choose another configured coverage to inspect its bundled public records.</p></div>}</section>{detailPanel}</section> : activeTab === "account" ? <section className="account-panel" aria-label="Local account settings"><section className="account-group"><p className="eyebrow">COVERAGE</p><h2>Selected coverage</h2><label className="coverage-control">Coverage<select value={coverageId} onChange={(event) => selectCoverage(event.target.value)}>{state.seed.locations.map((item) => <option value={item.id} key={item.id}>{item.label}</option>)}</select></label><p>Choose from Lamplighter’s bundled public-record coverage only.</p></section><section className="account-group"><p className="eyebrow">CURATED</p><h2>Local personalization</h2>{curatedControls}</section><section className="account-group"><p className="eyebrow">CHAT</p><h2>Local history</h2><p>Chat history is kept in this browser for this demo. Clearing it does not change Curated topics, ranking signals, coverage, or Chat availability.</p><button type="button" className="clear-chat" onClick={clearChatHistory}>Clear local Chat history</button>{accountStatus && <p className="account-status" role="status">{accountStatus}</p>}</section><section className="account-group"><p className="eyebrow">PRIVACY &amp; AVAILABILITY</p><h2>Browser-local demo</h2><p>Coverage, Curated preferences and signals, and Chat history stay in this browser. Lamplighter has no sign-in, sync, or server profile.</p><p>{live.available ? "Optional live Chat is available for grounded answers." : "Optional live Chat is unavailable; bundled grounded demos remain available."}</p></section></section> : <section className="chat-grid" aria-label={`Grounded chat for ${label}`}><section className="chat-thread"><div className="panel-heading"><p className="eyebrow">CLOSED EVIDENCE PACKET</p><h2>Ask about {label}</h2><p>{packet.length} surfaced records · citations open the source panel</p></div><section className="suggestion-list" aria-label="Suggested questions"><p className="eyebrow">SUGGESTED QUESTIONS</p>{suggestionQuestions.map((prompt) => <button type="button" key={prompt} onClick={() => void submitQuestion(undefined, prompt)} disabled={asking}>{prompt}</button>)}</section>
